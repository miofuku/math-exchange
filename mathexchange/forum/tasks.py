...
from mathexchange.accounts.tasks import create_messages
from mathexchange.emailer.tasks import send_email
from django.conf import settings
import time, random
from mathexchange.utils.decorators import task, timer
...
def notify_watched_tags(uid, extra_context):
    ...
    from mathexchange.forum.models import Post
    from django.conf import settings
...
#     """
#     from mathexchange.forum.models import Post
#     from mathexchange.forum import search
#     from django.conf import settings
#
...
def create_user_awards(user_id, limit=None):
    from mathexchange.accounts.models import User
    from mathexchange.forum.models import Award, Badge, Post
    from mathexchange.forum.awards import ALL_AWARDS
    from mathexchange.forum import util, auth
    from django.conf import settings
...
def batch_create_awards(limit=100):
    from mathexchange.accounts.models import User
    from mathexchange.forum import auth, models, util
...
def set_link_title(pk):
    ...
    from mathexchange.forum.models import SharedLink
...
@task
def spam_check(uid):
    from mathexchange.forum.models import Post, Log, delete_post_cache
    from mathexchange.accounts.models import User, Profile
    from mathexchange.forum.auth import db_logger
    ...
    try:
        from mathexchange.utils import spamlib
...
def herald_emails(uid):
    ...
    """
    from mathexchange.emailer.models import EmailSubscription, EmailGroup
    from mathexchange.forum.models import Post
    post = Post.objects.filter(uid=uid).first()
...
def notify_followers(sub_ids, author_id, uid, extra_context={}):
    ...
    """
    from mathexchange.forum.models import Subscription
    from mathexchange.accounts.models import Profile, User
    from mathexchange.forum.models import Post
    from django.conf import settings
